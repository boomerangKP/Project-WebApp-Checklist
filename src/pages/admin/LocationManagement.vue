...
const fetchData = async () => {
  ...
  try {
    const [locsRes, typesRes] = await Promise.all([
      ...
      supabase.from('locations').select('*, restroom_types(*)').order('locations_id', { ascending: false }),
...
const filteredList = computed(() => {
  return locations.value.filter(item => {
    ...
    const matchBuilding = filters.value.building ? item.locations_building === filters.value.building : true
    const matchFloor = filters.value.floor ? item.locations_floor === filters.value.floor : true
    const matchType = filters.value.type ? item.restroom_types_id === filters.value.type : true
...
const handleSave = async (formData) => {
  ...
  if (!formData.name || !formData.building || !formData.floor || !formData.code || !formData.typeId) {
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      ...
      const res = await supabase.from('locations').update(payload).eq('locations_id', formData.id).select().single()
      error = res.error
      resultData = res.data
    }
...
const handleDelete = async (id) => {
  const isConfirmed = await swalConfirm('ยืนยันการลบ?', 'ข้อมูลนี้จะหายไปถาวร ไม่สามารถกู้คืนได้', 'ลบเลย!')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <MapPin class="w-6 h-6 text-indigo-600" /> จัดการสถานที่
        </h1>
        <p class="text-gray-500 text-sm mt-1">รายการจุดตรวจทั้งหมดในระบบ ({{ filteredList.length }} รายการ)</p>
      </div>
