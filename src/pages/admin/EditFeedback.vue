...
const handleSave = async (formData) => {
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      const res = await supabase.from('feedback_topics').insert(payload).select().single()
      error = res.error
      resultData = res.data
    } else {
      const res = await supabase.from('feedback_topics').update(payload).eq('id', formData.id).select().single()
      error = res.error
      resultData = res.data
    }
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <MessageSquareQuote class="w-6 h-6 text-indigo-600" /> จัดการแบบประเมิน
        </h1>
