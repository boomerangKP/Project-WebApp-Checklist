...
const uniqueBuildings = computed(() =>
  [...new Set(locations.value.map((l) => l.locations_building).filter(Boolean))].sort()
);

const uniqueFloors = computed(() => {
  if (!filters.value.building) return [];
  const floors = locations.value
    .filter((l) => l.locations_building === filters.value.building)
...
const filteredLocations = computed(() => {
  return locations.value.filter((l) => {
    const matchSearch =
      ...
      l.locations_name.toLowerCase().includes(filters.value.search.toLowerCase()) ||
      l.locations_code.toLowerCase().includes(filters.value.search.toLowerCase());
    const matchBuilding =
      !filters.value.building || l.locations_building === filters.value.building;
    const matchFloor = !filters.value.floor || l.locations_floor === filters.value.floor;
    return matchSearch && matchBuilding && matchFloor;
  });
});

const selectedLocationsFull = computed(() =>
  locations.value.filter((l) => selectedIds.value.has(l.locations_id))
);
  ...
  <div class="h-[calc(100vh-9rem)] flex flex-col w-full">
    ...
    >
      <div>
        ...
        >
          <QrCode class="w-7 h-7 text-indigo-600 dark:text-indigo-400" /> พิมพ์ QR Code
          จุดตรวจ
        </h1>
        <p class="text-gray-500 dark:text-slate-400 text-sm mt-1">
          เลือกสถานที่ที่ต้องการพิมพ์สติ๊กเกอร์ QR Code
        </p>
      </div>
      ...
      <button
        ...
        class="flex-none flex items-center gap-2 px-5 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 dark:disabled:bg-slate-600 disabled:cursor-not-allowed text-white rounded-xl font-bold shadow-sm transition-all active:scale-95 ml-auto"
      >
