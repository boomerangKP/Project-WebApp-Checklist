    ...
    >
      <div class="bg-white pt-6 pb-2 px-6 sticky top-0 z-20 border-b border-gray-100">
        ...
        <div class="mb-4 flex flex-col items-center justify-center">
           ...
           />
           <p class="text-xs text-gray-500 font-bold">แบบประเมินสุขอนามัยและความสะอาด</p>
        </div>
        ...
        <Transition name="slide-fade" mode="out-in">
          ...
          <div v-if="!isCompleted" :key="currentStep" class="w-full">
            ...
            <div class="flex justify-center gap-2 mb-4">
              ...
              >
                <Star class="w-10 h-10 transition-colors" :class="getStarClass(currentTopic.id, i)" />
              </button>
            </div>
          ...
          <div v-else class="w-full text-center">
            ...
            <div class="bg-indigo-50 p-4 rounded-xl text-left">
              <div class="flex items-center justify-between mb-3">
                <h4 class="text-xs font-bold text-gray-500 uppercase">สรุปคะแนนของคุณ</h4>
              ...
              <div class="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                <div v-for="t in feedbackTopics" :key="t.id" class="text-xs flex items-center justify-between py-1 border-b border-indigo-100 last:border-0">
      ...
      <div class="p-6 bg-white border-t border-gray-100">
        <div class="flex gap-3 mb-3">
          <button v-if="currentStep > 0 || isCompleted" @click="prevStep" class="px-4 py-3 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
            <ChevronLeft class="w-6 h-6" />
          </button>

          <button v-if="!isCompleted" @click="nextStep" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl py-3 flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 active:scale-95 transition-all">
            ข้อต่อไป <ChevronRight class="w-5 h-5" />
          </button>

          <button v-else @click="submitFeedback" :disabled="submitting" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl py-3 flex items-center justify-center gap-2 shadow-lg shadow-green-200 active:scale-95 transition-all">
