...
onMounted(async () => {
  ...
  try {
    ...
    // ✅ แก้ไขตรงนี้: เพิ่ม || userRole === 'cleaner'
    if (session && (userRole === 'maid' || userRole === 'cleaner')) {
      // ✅ กรณี 1: เป็นแม่บ้าน หรือ พนักงานทำความสะอาด -> ไปหน้าส่งงาน
      router.replace({ 
        name: 'maid-job', 
        params: { id: location.locations_id } 
      })
    } else {
      // ✅ กรณี 2: เป็นลูกค้า หรือไม่ได้ Login หรือเป็น Admin -> ไปหน้าประเมินความพึงพอใจ
      router.replace({ 
        name: 'feedback', 
        params: { id: location.locations_id } 
      })
