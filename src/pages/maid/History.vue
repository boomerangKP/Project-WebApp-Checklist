...
const fetchLocations = async () => {
  const { data } = await supabase
    .from("locations")
    .select("locations_id, locations_name, locations_building")
...
const fetchHistory = async () => {
  try {
    ...
    let query = supabase
      .from("check_sessions")
      .select(
        `*, locations (locations_name, locations_building, locations_floor), restroom_types (restroom_types_name)`
      )
      .eq("employees_id", userStore.profile.employees_id)
      .order("created_at", { ascending: false });

    if (activeFilter.value.status !== "all") {
      if (activeFilter.value.status === "pass")
        query = query.in("check_sessions_status", ["pass", "fixed", "approved"]);
      else if (activeFilter.value.status === "fail")
        query = query.in("check_sessions_status", ["fail", "rejected"]);
      else if (activeFilter.value.status === "waiting")
        query = query.in("check_sessions_status", ["waiting", "in_progress"]);
    }
    if (activeFilter.value.location)
      query = query.eq("locations_id", activeFilter.value.location);
    ...
    .on(
      ...
      (payload) => {
        ...
        const index = historyItems.value.findIndex(
          (item) => item.check_sessions_id === updatedTask.check_sessions_id
        );
        ...
        if (index !== -1) {
          ...
          // ✅ เพิ่ม: อัปเดตเวลาตรวจด้วย (เพื่อให้หน้าจอแสดงเวลาทันที)
          historyItems.value[index].checked_at = updatedTask.checked_at;
        }
      }
    )
    .subscribe();
  ...
  <div class="min-h-screen bg-gray-50 pb-24 flex flex-col">
    ...
    <div class="bg-white p-4 shadow-sm fixed top-0 left-0 w-full z-20 flex items-center justify-between">
      ...
      <button
        ...
        class="p-2 -mr-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors relative"
      >
        <Filter class="w-6 h-6" />
        <span
          v-if="activeFilter.status !== 'all' || activeFilter.period !== 'all'"
          class="absolute top-2 right-2 w-2 h-2 bg-indigo-600 rounded-full border border-white"
        ></span>
      </button>
    </div>

    <div class="p-4 space-y-4 pt-20 flex-1 flex flex-col">

      <div
        v-if="loading"
        class="flex-1 flex flex-col items-center justify-center text-gray-400 min-h-[50vh]"
      >
