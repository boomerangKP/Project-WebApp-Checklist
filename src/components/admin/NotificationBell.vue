...
const getSlotName = (dateString) => {
  ...
  const match = timeSlots.value.find(
    (slot) => timeStr >= slot.time_slots_start && timeStr < slot.time_slots_end
  );
...
const getRoleConfig = (role) => {
  ...
  switch (r) {
    ...
    case "cleaner":
      return { type: "emoji", icon: "🧹", class: "bg-gray-200 text-lg dark:bg-gray-700" };
    default:
      return {
        type: "icon",
        icon: User,
...
<template>
  <div class="relative" ref="containerRef">
    <button
      ...
      class="relative p-2 rounded-full hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors text-gray-600 dark:text-gray-300 focus:outline-none"
    >
      <Bell class="w-6 h-6" />
      <span
        v-if="unreadCount > 0"
    ...
    >
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            class="group flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all"
          ...
          <div
            ...
            class="h-4 w-[1px] bg-gray-200 dark:bg-slate-600"
          ></div>
      ...
      >
        <div
          v-if="notifications.length === 0"
          class="p-10 text-center text-gray-400 dark:text-slate-500 text-sm flex flex-col items-center gap-2"
        >
          <div
            class="w-12 h-12 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center"
          >
        ...
        <div
          ...
          :class="
            noti.is_read
              ...
              : 'bg-[#E3EFFD] dark:bg-slate-700 border-blue-200 dark:border-slate-600 shadow-sm'
          "
        >
          <div
            v-if="!noti.is_read"
          ...
          <div class="flex gap-4">
              ...
              <div class="flex-shrink-0">
                ...
                >
                  <img
                    ...
                    :src="noti.employees.employees_photo"
                  ...
                  >
                    <span
                      v-if="getRoleConfig(noti.employees?.role).type === 'emoji'"
            ...
            <div class="flex-1 flex flex-col justify-center pl-1">
              ...
              <div
                class="flex items-center gap-1 text-[10px] text-gray-400 dark:text-slate-500 mt-1 truncate"
              >
                <MapPin class="w-3 h-3 flex-shrink-0" />
                <span class="truncate"
                  >อาคาร {{ parseMessage(noti.message).detail }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div
            class="absolute bottom-2 right-3 text-[10px] text-gray-400 dark:text-slate-500 font-medium group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"
          >
