...
const props = defineProps({
  ...
  selectedLocations: Array, // รับ object location เต็มๆ
  qrDataUrls: Object
})
...
<template>
  <div v-if="show" class="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm print:hidden">
    <div class="bg-white w-full max-w-5xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto p-8 bg-gray-100/50 custom-scrollbar">
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <div 
            ...
            class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center text-center relative overflow-hidden"
          >
            <div class="w-full h-1 bg-indigo-500 absolute top-0 left-0"></div>
            <template v-if="qrDataUrls[loc.locations_id]">
              <h4 class="font-bold text-gray-800 line-clamp-1 mt-2">{{ loc.locations_name }}</h4>
      ...
      <div class="px-6 py-4 border-t border-gray-100 bg-white flex justify-end gap-3">
        <button @click="$emit('close')" class="px-5 py-2.5 rounded-xl text-gray-600 hover:bg-gray-100 font-medium transition-colors">ยกเลิก</button>
        <button 
          @click="$emit('confirm')" 
          :disabled="isGenerating"
          class="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-200 flex items-center gap-2 transition-transform active:scale-95"
        >
